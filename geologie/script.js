document.addEventListener('DOMContentLoaded', () => {
  ...
  heroRects.forEach(r=>{ r.style.opacity = 0; r.style.transition = 'opacity .7s ease, transform .7s ease'; r.style.transform = 'translateY(12px)'; });
  setTimeout(()=>{ heroRects.forEach((r,i)=>{ setTimeout(()=>{ r.style.opacity = 1; r.style.transform='translateY(0)'; }, i*110); }); }, 300);

  /* ================= PLANCHE II — COUCHES INTERNES DE LA TERRE ================= */
  const EARTH_LAYERS = [
    { id:'croute',  nm:'Croûte terrestre', color:'var(--l-croute)',  rOuter:140, epaisseur:'5 à 70 km', temp:'0 à 1 000 °C', etat:'Solide',
    ...
    { id:'manteau', nm:'Manteau', color:'var(--l-manteau)', rOuter:122, epaisseur:'~2 900 km', temp:'1 000 à 3 700 °C', etat:'Solide, mais visqueux',
    ...
    { id:'externe', nm:'Noyau externe', color:'var(--l-externe)', rOuter:68, epaisseur:'~2 260 km', temp:'3 700 à 5 000 °C', etat:'Liquide (fer et nickel en fusion)',
      desc:"Ses mouvements de convection génèrent le champ magnétique terrestre, qui nous protège en permanence du vent solaire." },
    { id:'interne', nm:'Noyau interne', color:'var(--l-interne)', rOuter:34, epaisseur:'~1 220 km de rayon', temp:'~5 400 °C', etat:"Solide (la pression empêche la fusion malgré la chaleur)",
  ...
  earthSvg += `</svg>`;
  const legendHtml = EARTH_LAYERS.map(l=>`
  ...
  function selectEarthLayer(id){
    ...
    earthwrap.querySelectorAll('circle').forEach(c=>c.classList.toggle('selected', c.dataset.id===id));
    earthwrap.querySelectorAll('.earth-legend-item').forEach(el=>el.classList.toggle('active', el.dataset.id===id));
  ...
  const STRATA = [
    { nm:'Socle du Vishnu', color:'#4a4550', age:'~1 700 Ma — Précambrien', rk:'Roche métamorphique',
    ...
    { nm:'Grès de Tapeats', color:'#8a7355', age:'~525 Ma — Cambrien', rk:'Roche sédimentaire (grès)',
      desc:"Dépôt d'un ancien rivage marin peu profond, quand la mer a envahi le continent au Cambrien." },
    { nm:'Calcaire de Redwall', color:'#9c4a3a', age:'~340 Ma — Carbonifère', rk:'Roche sédimentaire (calcaire)',
    ...
    { nm:'Groupe de Supai', color:'#b5602d', age:'~315-285 Ma — Pennsylvanien-Permien', rk:'Grès et shale',
      desc:"Alternance de dépôts de plaines côtières et de deltas, témoins d'un niveau marin qui monte et descend." },
    { nm:'Shale de Hermit', color:'#c1573f', age:'~280 Ma — Permien', rk:'Roche sédimentaire (shale)',
      desc:"Boues rouges déposées sur une vaste plaine inondable, avant l'arrivée du désert qui suivra." },
    { nm:'Grès de Coconino', color:'#e8d9a0', age:'~275 Ma — Permien', rk:'Roche sédimentaire (grès éolien)',
    ...
    { nm:'Calcaire de Kaibab', color:'#d9c9a3', age:'~270 Ma — Permien', rk:'Roche sédimentaire (calcaire)',
  ...
  function selectStrat(i){
    const s = STRATA[i];
    stratStack.querySelectorAll('.strat-layer').forEach(l=>l.classList.toggle('active', +l.dataset.i===i));
  ...
  const PERIODS = [
    { nm:'Cambrien', color:'var(--e-cambrien)', dur:56, dates:'−541 à −485 Ma', desc:"« L'explosion cambrienne »&nbsp;: en quelques millions d'années, la plupart des grands groupes animaux actuels apparaissent." },
    { nm:'Ordovicien', color:'var(--e-ordovicien)', dur:41, dates:'−485 à −444 Ma', desc:"Diversification des invertébrés marins et premiers poissons sans mâchoire.", extinction:"Extinction de l'Ordovicien-Silurien (~444 Ma) — 2ᵉ plus grave de l'histoire." },
    { nm:'Silurien', color:'var(--e-silurien)', dur:25, dates:'−444 à −419 Ma', desc:"Premières plantes vasculaires et premiers animaux à conquérir la terre ferme." },
    { nm:'Dévonien', color:'var(--e-devonien)', dur:60, dates:'−419 à −359 Ma', desc:"« L'ère des poissons »&nbsp;: apparition des premiers tétrapodes, ancêtres de tous les vertébrés terrestres.", extinction:"Extinction du Dévonien supérieur (~372 Ma)." },
    { nm:'Carbonifère', color:'var(--e-carbonifere)', dur:60, dates:'−359 à −299 Ma', desc:"Immenses forêts marécageuses qui formeront plus tard le charbon. Insectes géants grâce à un air très riche en oxygène." },
    { nm:'Permien', color:'var(--e-permien)', dur:47, dates:'−299 à −252 Ma', desc:"Tous les continents sont réunis en un seul, la Pangée.", extinction:"« La Grande Extinction » (~252 Ma) — jusqu'à 96 % des espèces marines disparaissent. La plus grave de l'histoire." },
    { nm:'Trias', color:'var(--e-trias)', dur:51, dates:'−252 à −201 Ma', desc:"La vie se reconstruit lentement après la Grande Extinction&nbsp;: premiers dinosaures et premiers mammifères.", extinction:"Extinction Trias-Jurassique (~201 Ma), qui ouvre la voie aux dinosaures." },
    { nm:'Jurassique', color:'var(--e-jurassique)', dur:56, dates:'−201 à −145 Ma', desc:"L'âge d'or des dinosaures géants et apparition des premiers oiseaux." },
    { nm:'Crétacé', color:'var(--e-cretace)', dur:79, dates:'−145 à −66 Ma', desc:"Apparition des plantes à fleurs. Les dinosaures dominent jusqu'à la fin de la période.", extinction:"Extinction Crétacé-Paléogène (~66 Ma) — l'astéroïde de Chicxulub met fin au règne des dinosaures non-aviens." },
    { nm:'Paléogène', color:'var(--e-paleogene)', dur:43, dates:'−66 à −23 Ma', desc:"Radiation rapide des mammifères, qui occupent les niches laissées vides par les dinosaures." },
    { nm:'Néogène', color:'var(--e-neogene)', dur:20.4, dates:'−23 à −2,6 Ma', desc:"Refroidissement global et apparition des premiers hominidés bipèdes en Afrique." },
    { nm:'Quaternaire', color:'var(--e-quaternaire)', dur:2.6, dates:'−2,6 Ma à aujourd\'hui', desc:"Alternance de glaciations. Apparition puis expansion d'Homo sapiens, il y a environ 300 000 ans." },
  ];
  ...
  function selectEra(i){
    const p = PERIODS[i];
    erasTrack.querySelectorAll('.era-seg').forEach(s=>s.classList.toggle('active', +s.dataset.i===i));
  ...
  const ROCKS = [
    { id:'magmatique', nm:'Roche magmatique', pos:[150,55],
      formation:"Refroidissement et solidification du magma (en profondeur) ou de la lave (en surface).",
    ...
    { id:'sedimentaire', nm:'Roche sédimentaire', pos:[65,235],
      formation:"Accumulation, compaction puis cimentation de sédiments (débris de roches érodées, restes d'organismes).",
    ...
    { id:'metamorphique', nm:'Roche métamorphique', pos:[235,235],
      formation:"Transformation d'une roche existante sous l'effet de la chaleur et/ou de la pression, sans fusion complète.",
  ...
  ROCKS.forEach(r=>{
    rockSvg += `<g class="node" data-id="${r.id}">
      <circle cx="${r.pos[0]}" cy="${r.pos[1]}" r="42" fill="var(--paper)"/>
      <text x="${r.pos[0]}" y="${r.pos[1]-4}" text-anchor="middle">${r.nm.replace('Roche ','')}</text>
      <text x="${r.pos[0]}" y="${r.pos[1]+14}" text-anchor="middle" font-size="10" font-weight="400">clique-moi</text>
    </g>`;
  });
  rockSvg += `</svg>`;
  ...
  function selectRock(id){
    ...
    rockSvgWrap.querySelectorAll('.node').forEach(n=>n.classList.toggle('selected', n.dataset.id===id));
  ...
  const svgConv = `<svg viewBox="0 0 280 180" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;">
    ...
    <path d="M90,60 L120,60" stroke="var(--ink)" stroke-width="3" marker-end="url(#arrowhead-geo)"/>
    <path d="M190,40 L165,58" stroke="var(--ink)" stroke-width="3" marker-end="url(#arrowhead-geo)"/>
  ...
  const TECTO = [
    { key:'div', svg:svgDiv, badge:'Dorsale / rift', text:"Deux plaques s'écartent, le magma remonte pour combler l'espace et forme une <b>nouvelle croûte océanique</b>.", ex:"Dorsale médio-atlantique, rift est-africain." },
    { key:'conv', svg:svgConv, badge:'Subduction / collision', text:"Deux plaques se rapprochent&nbsp;: la plus dense <b>plonge sous</b> l'autre (subduction), ou les deux se compriment pour former des montagnes (collision).", ex:"Cordillère des Andes, chaîne de l'Himalaya." },
    { key:'trans', svg:svgTrans, badge:'Faille coulissante', text:"Deux plaques <b>glissent horizontalement</b> l'une contre l'autre, sans se rapprocher ni s'éloigner — souvent source de forts séismes.", ex:"Faille de San Andreas, en Californie." },
  ];
  const tectoPanels = document.getElementById('tectoPanels');
  tectoPanels.innerHTML = TECTO.map((t,i)=>`
  ...
  document.querySelectorAll('.ttab').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      document.querySelectorAll('.tecto-panel').forEach(p=>p.classList.remove('active'));
      document.querySelector(`.tecto-panel[data-panel="${btn.dataset.tab}"]`).classList.add('active');
    });
  });

  /* ================= PLANCHE VII — LEXIQUE ================= */
  const LEX = [
    { t:'Strate', d:'Couche de roche ou de sédiment, distincte de celles au-dessus et en dessous.' },
    { t:'Discordance', d:"Interruption dans une séquence de strates, signe d'une période d'érosion ou d'absence de dépôt." },
    { t:'Principe de superposition', d:'Dans une pile de strates non déformée, les couches du bas sont plus anciennes que celles du dessus.' },
    { t:'Fossile stratigraphique', d:"Espèce ayant vécu peu de temps mais très répandue, utile pour dater précisément une couche." },
    { t:'Magma', d:'Roche en fusion sous la surface. Devient de la lave une fois arrivée en surface.' },
    { t:'Subduction', d:"Plongement d'une plaque tectonique sous une autre, plus légère." },
    { t:'Dorsale océanique', d:'Chaîne de montagnes sous-marine où se forme continuellement une nouvelle croûte océanique.' },
    { t:'Craton', d:"Partie ancienne et stable d'un continent, qui résiste depuis très longtemps à la déformation." },
    { t:'Faille', d:'Fracture dans la roche le long de laquelle un déplacement a eu lieu.' },
    { t:'Ère / Période / Époque', d:'Subdivisions emboîtées de l\u2019échelle des temps géologiques, du plus large au plus précis.' },
  ];
